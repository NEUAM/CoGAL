...
class AdalamFilter:
    DEFAULT_CONFIG = {
        'area_ratio': 115,  # Ratio between seed circle area and image area. Higher values produce more seeds with smaller neighborhoods.
        'search_expansion': 3.5,  # Expansion factor of the seed circle radius for the purpose of collecting neighborhoods. Increases neighborhood radius without changing seed distribution
    ...
    def filter_matches(self, k1: torch.Tensor, k2: torch.Tensor,
                       ...
                       putative_matches21: torch.Tensor,
                       scores: torch.Tensor, scores21: torch.Tensor, distance: torch.Tensor ,mnn: torch.Tensor = None,
        ...
        with torch.no_grad():
            return adalam_core(k1, k2, fnn12=putative_matches,
                               fnn21=putative_matches21,
                               scores1=scores, scores21=scores21, distance=distance, mnn=mnn,
                                im1shape=im1shape,
                                im2shape=im2shape,
                                o1=o1, o2=o2,
                                s1=s1, s2=s2,
    ...
    def match_and_filter(self, k1, k2, d1, d2, im1shape=None, im2shape=None,
        ...
        distmat2 = dist_matrix(d2, d1, is_normalized=False)
        dd12, nn12 = torch.topk(distmat, k=2, dim=1, largest=False)  # (n1, 2)
        dd21, nn21 = torch.topk(distmat2, k=2, dim=1, largest=False)


        putative_matches = nn12[:, 0]
        putative_matches21 = nn21[:, 0]
        scores = dd12[:, 0] / dd12[:, 1].clamp_min_(1e-3)
        scores21 = dd21[:, 0] / dd21[:, 1].clamp_min_(1e-3)
        distance = dd12[:, 0]
        ...
        return self.filter_matches(k1, k2, putative_matches,putative_matches21, scores,scores21,distance, mnn,
